...
def predict(knn_model, vectorizer):
    # Read test data
    test_df = pd.read_csv('../data/washed_test_data.csv', encoding='ISO-8859-1')

    test_ground_truth = test_df.iloc[:, 0].copy()
    test_data = test_df.iloc[:, 5].copy()
    
    test_data_vec = vectorizer.transform(test_data)
    test_pred = knn_model.predict(test_data_vec)
    print("test set:")
    accuracy = accuracy_score(test_ground_truth, test_pred)
    precision = precision_score(test_ground_truth, test_pred)
    recall = recall_score(test_ground_truth, test_pred)
    f1 = f1_score(test_ground_truth, test_pred)
    print("Accuracy:", accuracy)
    print("Precision:", precision)
    print("Recall:", recall)
    print("F1 Score:", f1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test', action='store_true', default=False, help='train(True) or predict(False), default train')
    args = parser.parse_args()
    print(f"args.train is {not args.test}")
    main(args)
